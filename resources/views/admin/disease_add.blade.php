    ...
    <section class="mt-8 flex flex-col gap-5 z- bg-white p-4 rounded-md">
        ...
        <form action="{{ route('disease.store') }}" class="flex flex-col gap-2" method="POST">
            ...
            <div class="flex flex-col gap-1">
                ...
                <input class="border bg-transparent rounded-md px-3 py-2 w-[30rem]" type="text" id="penyakit" name="penyakit">
            </div>
            <div class="flex flex-col gap-1">
                <label class="text-sm font-semibold">Gejala</label>
                <div class="flex flex-wrap gap-2">
                    @foreach ($data as $item)
                        <input type="checkbox" id="gejala-{{$loop->index}}" name="gejala[]" value="{{ $item->gejala }}" class="hidden">
                        <label for="gejala-{{$loop->index}}" class="px-6 py-2 bg-gray-300 rounded-md cursor-pointer text-black font-semibold"
            ...
            <div class="flex flex-col gap-1">
                ...
                <textarea class="border bg-transparent rounded-md px-3 py-2 w-[30rem]" type="text" id="deskripsi" name="deskripsi"></textarea>
            </div>
            <div class="flex flex-col gap-1">
                <label for="penanganan" class="text-sm font-semibold">Penanganan</label>
                <textarea class="border bg-transparent rounded-md px-3 py-2 w-[30rem]" type="text" id="penanganan" name="penanganan"></textarea>
            </div>
