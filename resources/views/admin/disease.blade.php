<x-admin.layout>
    ...
    <section class="mt-8 flex flex-col gap-5 z-0">
        <div class="bg-white flex items-center justify-between p-4 rounded-md">
            <h3 class="font-semibold text-lg">Daftar Penyakit</h3>
            <a href="/admin/disease-add" class="text-blue-500 hover:text-blue-800 text-sm font-semibold">Tambah Penyakit</a>
        </div>

        <div class="z-0">
            {{ $data->links() }}
        </div>
        <table class="border mr-4 w-full mt-4 border-gray-300 bg-white">
            <thead>
            ...
            <tbody>
                ...
                @foreach ($data as $item) 
                    <tr class="border">
                        ...
                        <td class="p-2">{{$item->penyakit}}</td>
                        <td class="p-2">{{ $item->gejala }}</td>
                        <td class="p-2">{{ Str::limit($item->deskripsi, 20) }}</td>
