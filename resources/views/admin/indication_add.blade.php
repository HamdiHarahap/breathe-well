<x-admin.layout>
    ...
    <section class="mt-8 flex flex-col gap-5 z- bg-white p-4 rounded-md">
        ...
        <form action="{{ route('indication.store') }}" class="flex flex-col gap-2" method="POST">
            @csrf
            <div class="flex flex-col gap-1">
                <label for="gejala" class="text-sm font-semibold">Gejala</label>
                <input class="border bg-transparent rounded-md px-3 py-2 w-[30rem]" type="text" id="gejala" name="gejala">
            </div>
            <div class="flex flex-col gap-1">
                <label for="pertanyaan" class="text-sm font-semibold">Pertanyaan</label>
                <textarea class="border bg-transparent rounded-md px-3 py-2 w-[30rem]" type="text" id="pertanyaan" name="pertanyaan"></textarea>
            </div>
